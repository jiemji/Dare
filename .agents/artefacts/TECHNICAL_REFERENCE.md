...
| **Card Long** | `.card-long` | 100% | Horizontal | Threats, Events (Compact) |
| **Card Extended**| `.card-extended` | 400px+ (Flex) | Vertical | Business Processes, Forms |
| **Table Square** | `.table-square` | Max 400px | N/A | Scales, Small Matrixes |
...
- **Persistence**: Data is saved to `localStorage` under the key `dare-data`.
...
- The `pageStructure` object in `app.js` defines the mapping between IDs and HTML file paths.
